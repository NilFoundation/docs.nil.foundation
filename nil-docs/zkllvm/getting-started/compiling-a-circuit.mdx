...
# Compiling and verifying a circuit
...
Take the `.cpp` or `.rs` file with the circuit and place it inside either the `./examples/cpp` (C++) the `./examples/rust` directory.
...
<Tabs groupId='language'>
  ...
  <TabItem value='rust' label='Rust'>
    ...
    ```
    add_example_with_proving({name_of_build_target} SOURCES{name_of_file_with_code.rs} INPUT {name_of_public_inputs_file.inp} CURVE_TYPE
    bls12381)
    ```
...
After a couple of seconds, the `./build/examples/cpp` (C++) or the `.build/examples/rust` (Rust) directory should contain the following.
