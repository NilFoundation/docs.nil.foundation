...
All pre-built binaries discussed below are only supported on x86-x64 systems.
...
To use the original bootstrap system (`x.py`) supplied by Rust, build `zkllvm` first.
