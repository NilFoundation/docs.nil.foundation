...
This step will add `zkllvm` to the list of installed packages but it will not build all of the required components including `assigner`. This can only be achieved by performing a full installation from sources.

## Building zkLLVM from sources
...
There are two possible ways to install and build the Rust compiler.

- Use a pre-binary installation
- Build the compiler from source

#### Using a pre-built binary

If `rustup` is already installed, it is possible to install the Rust compiler from a pre-built binary which is easier and faster compared to building it from source.

Install `rslang` by executing the following command.
...
Regardless of the chosen installation method, the Rust compiler should now be accessible as follows.
...
To use the default build system supplied by Rust, build `zkllvm` first.
