# Using const size containers

Using const size containers in circuits is preferable compared to using dynamic size containers.

When compiled into a circuit, const size containers carry the same costs regardless of the memory type they are using. This means that const size containers offer incresed reusability and efficiency across memory types and platforms.

Consider the following examples in which const size containers are used to build a four-layer Merkle tree.
...
<Tabs groupId='language'>
  ...
  <TabItem value='rust' label='Rust'>
    ...
    for leaf_index in 0..2 {
        layer_4_leaves[leaf_index] = hash(layer_3_leaves[2 * leaf_index], layer_3_leaves[2 * leaf_index + 1]);
    }
