...
2. Execution shards survey other execution shards to determine the shard where the message should be included in a block
3. The message goes to the mempool of an execution shard
4. The message is picked up by a collator that bundles it with other message and sends the resulting batch for execution
5. The message is executed and is sent into a block
6. The block passes local consensus while the shard generates a ZKP
7. The block with the message is sent into storage and the message reaches soft finality
