...
Internal messages are those messages that are spawned by smart contracts within one shard and are then sent to addresses located within the same shard. When a Contract 1 (deployed on Shard 1) calls a function in Contract 2 (also deployed on Shard 2), an internal message is recorded.

External messages can originate in two possible cases:

1. When any entity that exists outside of the cluster (e.g., a dApp) sends a message to any smart contract on the network
2. When a smart contract deployed on one shard calls a different smart contract deployed on another shard

Imagine a case where Contract 1 is deployed on Shard 1 and Contract 2 is deployed on Shard 2. When Contract 1 calls a function insde Contract 2, an external message is recorded. From the perspective of Contract 2, the function call is made by an entity that exists outside of its network.

[**Click here**](../guides/architecture/ext-messages) to read a more detailed explanation of how external messages function and how they can be handled. 
