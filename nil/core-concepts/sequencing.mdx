...
As =nil; adopts the proposer-builder separation (PBS) model, the key principle of its approach to sequencing lies in building a network of builders and searchers competing for maximum extractable value (MEV) in relay auctions.

:::tip[Justification]

The sequencing structure of =nil; is fully compatible with Ethereum, which means that builders and searchers from L1 can easily participate in =nil;. This boosts protocol stability and liveness without sacrificing sovereignty.
...
Additionally, execution shards are merged if their builders are inactive and their block count is below half of the block capacity of these shards. This prevents another possible exploit where a participant may specifically target underutilized shards to attain rewards reliably.

:::note[Merge events]

The merge event only threatens significant changes to searchers and builders if there is a sizeable list of pre-prepared blocks. In any other case, a merge will not entail any sudden changes to the blocks constructed in the shards being merged. However, relays will need to reapprove signatures in the newly formed committees to continue delivering blocks. 
...
There are several reasons dictating why the consensus shard follows distinct approach to sequencing.

* Lack of MEV issues. at the consensus shard level, there is no more value to extract from sequencing
