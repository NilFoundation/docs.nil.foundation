...
In =nil;, ensuring the availability of L2 data is delegated to the syncronization committee.
...
4. If the hash attains $2/3 + 1$ votes, the hash, the state difference, and the aggregate signature are composed into an Ethereum data availability transaction
...
If there are too few data availability or state-proof transactions, the synchronization committee can decide to send them directly to builders and searchers as a bundle.

## At the consensus shard level
