...
`ASYNC_CALL` is a pre-compile for the `asyncCall()` function that allows for async execution of contract functions:
...
[**Click here**](../../core-concepts/shards-parallel-execution##async-execution) for a primer on async execution and [**here**](../../getting-started/essentials/handling-async-execution) for a tutorial on using `asynccall()`.
...
`SEND_MESSAGE` is a pre-compile for the `sendmsg()` function:

```solidity showLineNumbers
function sendmsg(uint g, bytes memory message) internal
```
