...
Each group of validators (a committee) is responsible for overseeing a particular shard. A committee regularly monitors the shards that are located close to the shard that they are supposed to govern. 

If the committee tracks an ongoing message, it must compute the shard where the message should go based on the information included in the message. If this shard is the same as the shard that the committee is overseeing, the message must be included in the block currently produced by the shard.
