...
Contract 1 is a basic contract for voting:
...
contract Elections {
    ...
    function request_vote(address dst, string candidate) public payable {
        dst.async_call(gasleft(), msg.value, abi.encodeWithSignature("verify_vote()"));
        emit CallCompleted(dst);
    }

    function add_vote(string candidate)
}
```

Contract 2 hold some verification 
