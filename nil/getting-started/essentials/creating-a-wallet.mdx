...
The wallet can be 'topped up' by using the `wallet top-up AMOUNT` command. Note that using this command disruptively (e.g., by requesting a 'top up' for an infinite amount of tokens) may destabilize the network.
