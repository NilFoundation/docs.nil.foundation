...
```bash
nil_cli wallet send-message CONTRACT_ADDRESS METHOD_NAME [ARGS] --abi METHOD_NAME
```
...
The `waitTillCompleted()` function encorces waiting for the results of the execution of the message whose hash is passed as an argument. It is crucial for avoiding null-type errors.
