...
This tutorial explains how =nil; handles tokens 
...
```bash
./build/bin/nil_cli minter create-currency OWNER_ADDRESS AMOUNT NAME --withdraw
```

To mint a currency:

```bash
./build/bin/nil_cli minter create-currency OWNER_ADDRESS AMOUNT
```

To send an existing currency (with some amount already minted) from a minter to another contract:

```bash
./build/bin/nil_cli minter withdraw-currency OWNER_ADDRESS AMOUNT RECEIVER_ADDRESS
```
...
await faucet.withdrawToWithRetry(walletTwoAddress, 10_000_000n);
