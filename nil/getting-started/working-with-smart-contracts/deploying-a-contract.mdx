...
In order to be fully compatible with internal deployments via some developer tools (such as the =nil; CLI, the contract constructor has to be set as `payable`).
...
```bash
nil_cli wallet deploy ./Manufacturer/Manufacturer.bin PUBKEY ADDRESS --abi ./Manufacturer/Manufacturer.abi --amount 5000000 --shard-id 1
```

```bash
nil_cli wallet deploy ./Retailer/Retailer.bin --abi ./Retailer/Retailer.abi --amount 5000000 --shard-id 2
```
...
module.exports = buildModule("Manufacturer17", (m: any) => {

  const pubkey = PUBKEY;
  const owner = ADDRESS;

  const manufacturer = m.contract("Manufacturer", [pubkey, owner]);
...
module.exports = buildModule("RetailerSeven", (m: any) => {
...
```bash
npx hardhat ignition deploy ./ignition/modules/Manufacturer.ts --network nil_cluster
```

```bash
npx hardhat ignition deploy ./ignition/modules/Retailer.ts --network nil_cluster
```
