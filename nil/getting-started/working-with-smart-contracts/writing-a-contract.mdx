...
Contract 2 is the retailer. The contract can order Contract 1 to order new products by making an async call.
...
contract Manufacturer {
    ...
    function setRetailerContractAddress(address _retailerContractAddress) public onlyExternal {
        if (isRetailerAddressSet == false) {
