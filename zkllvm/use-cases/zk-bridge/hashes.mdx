...
<Tabs groupId='language'>
  ...
  <TabItem value='rust' label='Rust'>
    ```rust
    fn is_same(x: Block, y: Block) -> bool {
        x[0] == y[0] && x[1] == y[1]
    }
...
<Tabs groupId='language'>
  ...
  <TabItem value='rust' label='Rust'>
    ...
    fn verify_protocol_state_proof(
        ...
        unconfirmed_blocks: [BlockDataType; 64],
    ) -> bool {
        for i in 0..64 {
            if i == 0 {
...
The `block_data_type` / `BlockDataType` aggregate type can be expressed as follows:
...
The public input for the circuit would look as follows:
