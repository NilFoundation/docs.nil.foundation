...
`crypto3` already depends on Boost. If an alternative library (that does not depend on Boost) is used, add Boost to the CMake configuration:
...
If this feature is enabled for a project, built-in zkLLVM types are used to represent curves, fields, and other elements. If this feature is disabled, type definitions from the `arkworks` project are used.
